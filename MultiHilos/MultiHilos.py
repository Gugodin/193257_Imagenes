from imgurpython import ImgurClient
 
import os
import urllib.request
from multiprocessing import Pool
from concurrent.futures import ThreadPoolExecutor  
import timeit
 
secreto_cliente = "5f8c3cce299db5e26a2eb96b0b7809a82805c9ad"
id_cliente = "bfa0e227a1c5643"
 
cliente = ImgurClient(id_cliente, secreto_cliente)
 
# Metodo para la descarga url imagen
# Datos necesarios del metodo
# Nombre de la imagen => yntdWAr
# Formato de la imagen => png
 
 
def descarga_url_img(link):
   print(link)
   # Con esto ya podemos obtener el corte de la url imagen
   nombre_img = link.split("/")[3]
   formato_img = nombre_img.split(".")[1]
   nombre_img = nombre_img.split(".")[0]
   print(nombre_img, formato_img)
   url_local = "/PROYECTOS/Uni/Cuatri 7/Corte 2/Concurrente/Tarea ult/Imagen/{}.{}"
   #Guardar nne local las imagenes
   urllib.request.urlretrieve(link, url_local.format(nombre_img, formato_img))
 
urls = []
def llenarUrls():
   id_album = "bUaCfoz"
   imagenes = cliente.get_album_images(id_album)
 
   for imagen in imagenes:
       urls.append(imagen.link)

def multiHilos():
   print('Multi hilos')
   # Max_workes son la cantidad de hilos que se ejecutaran en relacion a la variable iterable que le pases (urls)
   with ThreadPoolExecutor(max_workers=len(urls)) as executor:
      executor.map(descarga_url_img,urls)



def multiProcesos():
   print('Multi procesos')
   #Multi procesos, en donde se marca la cantidad total de procesos que se va a ejecutar
   with Pool(len(urls)) as p:
      p.map(descarga_url_img,urls)

def estructurado():
   print('Proceso estructurado')

   for i in range(len(urls)):
      descarga_url_img(urls[i])
  

 
 
if __name__ == "__main__":
   llenarUrls()

   print("Tiempo de descarga multi hilos {}".format(timeit.Timer(multiHilos).timeit(number=1)))
   print("Tiempo de descarga multi procesos {}".format(timeit.Timer(multiProcesos).timeit(number=1)))
   print("Tiempo de descarga proceso estructurado {}".format(timeit.Timer(estructurado).timeit(number=1)))
